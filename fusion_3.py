...
class AttentionGate(nn.Module):
    ...
    def forward(self, x):
        ...
        scale = torch.clamp(torch.sigmoid_(x_out), min=0.1, max=0.9)  # 限制权重范围
        return x * scale
...
def new_attention(tensor):
    # channel_attention = ChannelAttention(tensor.size(1)).to(tensor.device)
    new_attention = TripletAttention().to(tensor.device)
    # f_channel = channel_attention(tensor)
    f_new = new_attention(tensor)
